...
import { PublicElectricityView } from './components/PublicElectricityView';
import PublicExpenseSubmit from './pages/PublicExpenseSubmit';
...
const MainApp: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const [allBills, allProjects, allBillTypes, allCompanies] = await Promise.all([
        api.getBills(),
        api.getProjects(),
        api.getBillTypes(),
        api.getCompanies()
      ...
      setCompanies(allCompanies.map(c => c.companyName));
    } catch (err) {
  ...
  useEffect(() => {
    if (user?.role === 'EA' && (activeTab === 'dashboard' || activeTab === 'bills')) {
  ...
  useEffect(() => {
    ...
    const handleBillUpdated = (updatedBill: Bill) => {
      setBills(prev => {
        const filtered = prev.filter(b => (b.id || b._id) !== (updatedBill.id || updatedBill._id));
        return [updatedBill, ...filtered];
      });
    };
  ...
  const handleSaveBill = async (data: Partial<Bill>) => {
    try {
      ...
      const savedBill = await api.saveBill(data, user ? { name: user.name, role: user.role } : undefined);
      setIsModalOpen(false);
      setEditingBill(null);
      toast.success(isUpdate ? 'Bill updated successfully!' : 'Bill saved successfully!');
      
      const id = savedBill.id || (savedBill as any)._id;
      setBills(prev => {
        const otherBills = prev.filter(b => (b.id || (b as any)._id) !== id);
        return [savedBill, ...otherBills];
      });
    } catch (err) {
  ...
  const handleMarkPaid = async (updatedBill: Bill) => {
    try {
      const savedBill = await api.saveBill(updatedBill, user ? { name: user.name, role: user.role } : undefined);
      toast.success('Bill marked as paid');
      const id = savedBill.id || (savedBill as any)._id;
      setBills(prev => prev.map(b => (b.id || (b as any)._id) === id ? savedBill : b));
    } catch (err) {
  ...
  const confirmDeleteBill = async () => {
    ...
    setIsDeleting(true);
    try {
      await api.deleteBill(billToDelete, user ? { name: user.name, role: user.role } : undefined);
      setBills(prev => prev.filter(b => (b.id || (b as any)._id) !== billToDelete));
  ...
  const handleTallySave = async (tallyData: any) => {
    if (!editingBill) return;
    try {
      const updatedBill = await api.tallyEntry(
        editingBill._id || editingBill.id!,
        tallyData.narration || 'Tally Entry Completed',
        user?.name,
        user?.role,
        tallyData
      );
      ...
      setBills(prev => prev.map(b => (b.id || (b as any)._id) === id ? updatedBill : b));
    } catch (err) {
      console.error('Tally entry error:', err);
      toast.error('Failed to record Tally entry');
      throw err;
    }
  };

  const handleWorkflowAction = async (remarks: string, proofFile?: File, extraDetails?: { bankName?: string, upiMode?: string, upiReference?: string, amount?: number, paymentDate?: string }) => {
    if (!editingBill) return;
    ...
    try {
      ...
      if (workflowActionType === 'verify') {
      ...
      } else if (workflowActionType === 'confirm') {
        let proofUrl = '';
        let proofName = '';
        ...
        toast.success('Tally entry completed successfully');
      } else {
      ...
      const billId = updatedBill.id || (updatedBill as any)._id;
      setBills(prev => prev.map(b => (b.id || (b as any)._id) === billId ? updatedBill : b));
  ...
  const filteredBills = bills.filter(bill => {
    // Role based filtering for ACCOUNT_EXECUTIVE_2
    if (user?.role === 'ACCOUNT_EXECUTIVE_2') {
      const isElectricity = bill.utilityType === 'Electricity';
      const isTelecom = bill.utilityType === 'Telecom' || bill.utilityType === 'Data (Internet)' || bill.utilityType === 'Landline' || bill.utilityType === 'Mobile Recharge' || bill.category === 'telecom';
      const isSolar = bill.utilityType === 'Solar Bill';
      if (!(isElectricity || isTelecom || isSolar)) return false;
    }
    ...
    const matchesCompany = filters.company ? bill.companyName === filters.company : true;
    ...
    const matchesMonth = filters.month ? new Date(bill.billDate).getMonth() + 1 === parseInt(filters.month) : true;
    const matchesYear = filters.year ? new Date(bill.billDate).getFullYear() === parseInt(filters.year) : true;
    ...
    return matchesSearch && matchesProperty && matchesCompany && matchesType && matchesStatus && matchesDate && matchesRegion && matchesMonth && matchesYear;
  });

  const roleBills = user?.role === 'ACCOUNT_EXECUTIVE_2' ? bills.filter(bill => {
    const isElectricity = bill.utilityType === 'Electricity';
    const isTelecom = bill.utilityType === 'Telecom' || bill.utilityType === 'Data (Internet)' || bill.utilityType === 'Landline' || bill.utilityType === 'Mobile Recharge' || bill.category === 'telecom';
    const isSolar = bill.utilityType === 'Solar Bill';
    return isElectricity || isTelecom || isSolar;
  }) : bills;

  // Calculate Dashboard Stats
  const todayStr = new Date().toISOString().split('T')[0];
  const isOverdueCheck = (b: any) => {
    if (['Payment Confirmed', 'Tally Entry', 'Paid'].includes(b.status || '')) return false;
    return b.status === 'Overdue' || (b.dueDate && b.dueDate < todayStr);
  };

  const dashboardStats = [
    { status: 'PAID', count: roleBills.filter(b => b.status === 'Payment Confirmed' || b.status === 'Tally Entry' || b.status === 'Paid').length, total_amount: roleBills.filter(b => b.status === 'Payment Confirmed' || b.status === 'Tally Entry' || b.status === 'Paid').reduce((sum, b) => sum + b.amount, 0) },
    { status: 'OVERDUE', count: roleBills.filter(isOverdueCheck).length, total_amount: roleBills.filter(isOverdueCheck).reduce((sum, b) => sum + b.amount, 0) },
    { status: 'PENDING', count: roleBills.filter(b => !['Payment Confirmed', 'Tally Entry', 'Paid'].includes(b.status || '')).length, total_amount: roleBills.filter(b => !['Payment Confirmed', 'Tally Entry', 'Paid'].includes(b.status || '')).reduce((sum, b) => sum + b.amount, 0) },
    { status: 'IN_PROCESS', count: roleBills.filter(b => !isOverdueCheck(b) && !['Payment Confirmed', 'Tally Entry', 'Paid'].includes(b.status || '')).length, total_amount: roleBills.filter(b => !isOverdueCheck(b) && !['Payment Confirmed', 'Tally Entry', 'Paid'].includes(b.status || '')).reduce((sum, b) => sum + b.amount, 0) },
    { status: 'CLOSED', count: 0, total_amount: 0 },
  ];

  const financialStats = {
    total_pending: roleBills.filter(b => !['Payment Confirmed', 'Tally Entry', 'Paid'].includes(b.status || '')).reduce((sum, b) => sum + b.amount, 0),
    total_paid: roleBills.filter(b => ['Payment Confirmed', 'Tally Entry', 'Paid'].includes(b.status || '')).reduce((sum, b) => sum + b.amount, 0),
  };

  const byPriority = [
    { priority: 'URGENT', count: roleBills.filter(b => b.priority === 'Urgent').length },
    { priority: 'CRITICAL', count: roleBills.filter(b => b.priority === 'High').length },
    { priority: 'NORMAL', count: roleBills.filter(b => b.priority === 'Normal').length },
    { priority: 'LOW', count: roleBills.filter(b => b.priority === 'Low').length },
  ];

  const byType = Array.from(new Set(roleBills.map(b => b.utilityType))).map(type => ({
    type,
    count: roleBills.filter(b => b.utilityType === type).length
  }));
  ...
  const handleExportExcel = () => {
    try {
      ...
      writeFile(wb, `Bills_Report_${new Date().toISOString().split('T')[0]}.xlsx`);
  ...
  const handleExportPdf = () => {
    try {
      ...
      const tableColumn = ["Bill No", "Property", "Category", "Amount", "Status", "Due Date"];
      ...
      // Improved helper for autoTable compatibility
  ...
  return (
    <div className="flex h-screen bg-content-bg overflow-hidden transition-colors duration-300">
      <Sidebar 
        activeTab={activeTab} 
      ...
      <main className="flex-1 flex flex-col min-w-0 h-full overflow-hidden">
        <Header activeTab={activeTab} onMenuClick={handleMenuClick} />
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6">
          {activeTab === 'dashboard' && (
            <motion.div 
              ...
              className="space-y-6"
            >
              <UnifiedDashboard onNavigate={setActiveTab} user={user} />
            </motion.div>
          )}
          ...
          {activeTab === 'bills' && (
            ...
            >
              <BillManagementHeader 
                ...
                companies={Array.from(new Set(bills.map(b => b.companyName).filter(Boolean)))}
                onAddBill={() => {
                  setEditingBill(null);
                  setIsModalOpen(true);
                }}
              ...
              <SummaryCards bills={filteredBills} />
          ...
          {activeTab === 'electricity' && (
            <motion.div 
              ...
              animate={{ opacity: 1, y: 0 }}
            >
              <ElectricityModule projects={projects} isLoadingProjects={isLoading} allBills={bills} user={user} />
            </motion.div>
          )}
          ...
          {activeTab === 'telecom' && (
            <motion.div 
              ...
              animate={{ opacity: 1, y: 0 }}
            >
              <TelecomModule projects={projects} isLoadingProjects={isLoading} allBills={bills} user={user} />
            </motion.div>
          )}
          ...
          {activeTab === 'government' && (
            <motion.div 
              ...
              animate={{ opacity: 1, y: 0 }}
            >
              <GovernmentBillsModule projects={projects} isLoadingProjects={isLoading} allBills={bills} user={user} />
            </motion.div>
          )}
          ...
          {activeTab === 'solar' && (
            <motion.div 
              ...
              animate={{ opacity: 1, y: 0 }}
            >
              <SolarModule projects={projects} isLoadingProjects={isLoading} allBills={bills} user={user} />
            </motion.div>
          )}
          ...
          {activeTab === 'insurance' && (
            <motion.div 
              ...
              animate={{ opacity: 1, y: 0 }}
            >
              <InsuranceModule projects={projects} isLoadingProjects={isLoading} allBills={bills} user={user} />
            </motion.div>
          )}
          ...
          {activeTab === 'pollution' && (
            <motion.div 
              ...
              animate={{ opacity: 1, y: 0 }}
            >
              <PollutionControlModule projects={projects} isLoadingProjects={isLoading} allBills={bills} user={user} />
            </motion.div>
          )}
          ...
          {activeTab === 'expenses' && (
            <motion.div 
              ...
              animate={{ opacity: 1, y: 0 }}
            >
              <OtherExpensesModule projects={projects} isLoadingProjects={isLoading} allBills={bills} user={user} />
            </motion.div>
          )}
          ...
          {activeTab !== 'bills' && activeTab !== 'reports' && activeTab !== 'dashboard' && activeTab !== 'settings' && activeTab !== 'electricity' && activeTab !== 'telecom' && activeTab !== 'government' && activeTab !== 'solar' && activeTab !== 'insurance' && activeTab !== 'pollution' && activeTab !== 'expenses' && !activeTab.startsWith('category:') && (
      ...
      />
      {user?.role === 'ADMIN' && <ChatBot bills={bills} />}
    </div>
  );
};
          ...
          <NotificationProvider>
            <SocketProvider>
              ...
              <Routes>
                ...
                <Route path="/public/expense-submit" element={<PublicExpenseSubmit />} />
                <Route path="/public/electricity" element={<PublicElectricityView />} />
