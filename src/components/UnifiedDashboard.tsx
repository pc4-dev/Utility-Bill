...
export const UnifiedDashboard: React.FC<UnifiedDashboardProps> = ({ onNavigate }) => {
  ...
  const [filter, setFilter] = useState({ category: 'all', status: 'all', company: 'all' });
  ...
  const ROLE_MODULES_MAPPING: Record<string, { label: string, categories: string[], modules: string[] }> = {
    ADMIN: { 
      label: 'Administrator', 
      categories: ['utility', 'telecom', 'insurance', 'government_tax', 'government_compliance', 'expense', 'other'],
      modules: ['Electricity', 'Solar', 'Telecom', 'Insurance', 'Government Tax', 'Pollution Control', 'Other Expenses']
    },
    MANAGER: { 
      label: 'Manager', 
      categories: ['utility', 'telecom', 'insurance', 'government_tax', 'government_compliance', 'expense', 'other'],
      ...
      const allowedModules = Object.entries(user.permissions)
        ...
        .map(([id, _]) => {
          ...
          switch(id) {
            ...
            case 'expenses': return 'Other Expenses';
            default: return id;
          }
        });
  ...
  const fetchData = async () => {
    try {
      ...
      const summary = await api.getDashboardSummary(
        isRestricted ? userPermissions.categories : undefined,
        filter.company
      );
      setData(summary);
  ...
  }, [socket, user?.role, filter.company]);
  ...
  const getModuleIcon = (mod: string) => {
    switch (mod) {
      ...
      case 'Other Expenses': return <Receipt className="w-5 h-5" />;
  ...
  const feedCounts = useMemo(() => {
    ...
    const isOverdue = (b: any) => {
      if (['Paid', 'Payment Confirmed', 'Tally Entry'].includes(b.status || '')) return false;
      return b.status === 'Overdue' || (b.dueDate && b.dueDate < todayStr);
    };
    return {
      total: filteredBills.length,
      inProgress: filteredBills.filter((b: any) => !['Paid', 'Payment Confirmed', 'Tally Entry'].includes(b.status || '') && !isOverdue(b)).length,
      overdue: filteredBills.filter(isOverdue).length,
      paid: filteredBills.filter((b: any) => ['Paid', 'Payment Confirmed', 'Tally Entry'].includes(b.status || '')).length
    };
  }, [filteredBills]);
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      {user?.role !== 'DATA_ENTRY' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
          ...
          <SummaryCard 
            ...
            value={(displayData?.statusCounts?.['Payment Confirmed'] || 0) + (displayData?.statusCounts?.['Tally Entry'] || 0) + (displayData?.statusCounts?.Paid || displayData?.statusCounts?.PAID || 0)}
            icon={<CheckCircle2 className="w-6 h-6" />}
            trend="Confirmed & Recorded"
            color="green"
          />
          <SummaryCard 
            title="In Progress" 
            value={(displayData?.totalDocuments || 0) - ((displayData?.statusCounts?.['Payment Confirmed'] || 0) + (displayData?.statusCounts?.['Tally Entry'] || 0) + (displayData?.statusCounts?.Paid || displayData?.statusCounts?.PAID || 0))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        {/* Expense Breakdown */}
        {displayData?.expenseBreakdown?.length > 0 && (
          <div className="lg:col-span-1 bg-white dark:bg-slate-900 p-8 rounded-3xl border border-gray-100 dark:border-slate-800 shadow-sm">
            <h3 className="text-xl font-black text-text-primary tracking-tight mb-8">Expenses</h3>
            <div className="h-[240px] mb-6">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    data={displayData.expenseBreakdown.map((e: any) => ({ name: e._id, value: e.amount }))}
                    innerRadius={60}
                    outerRadius={80}
                    paddingAngle={5}
                    dataKey="value"
                  >
                    {displayData.expenseBreakdown.map((_: any, index: number) => (
                      <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />
                    ))}
                  </Pie>
                  <Tooltip 
            ...
            <div className="space-y-3">
              {displayData.expenseBreakdown.slice(0, 5).map((e: any, idx: number) => (
                <div key={e._id} className="flex items-center justify-between group">
                  <div className="flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full" style={{ backgroundColor: COLORS[idx % COLORS.length] }} />
                    <span className="text-xs font-bold text-text-secondary capitalize">{e._id}</span>
                  </div>
                  <span className="text-xs font-black text-text-primary">{formatCurrency(e.amount)}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className={cn("bg-white dark:bg-slate-900 p-8 rounded-3xl border border-gray-100 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col", user?.role === 'DATA_ENTRY' ? "lg:col-span-4" : "lg:col-span-3")}>
            ...
            <div className="flex flex-wrap items-center gap-3">
              <div className="relative">
                ...
                <input 
                  ...
                  className="pl-9 pr-4 py-2 h-10 bg-gray-50 dark:bg-slate-800 border-none rounded-xl text-xs font-bold text-text-primary focus:ring-2 focus:ring-orange-500/20 transition-all w-[200px]"
                  value={searchTerm}
              ...
              <div className="relative">
                ...
                <select 
                  className="pl-9 pr-8 h-10 bg-gray-50 dark:bg-slate-800 border-none rounded-xl text-[11px] font-bold text-text-secondary focus:ring-2 focus:ring-orange-500/20 transition-all appearance-none cursor-pointer"
                  value={filter.status}
              ...
              <div className="relative">
                ...
                >
                  ...
                  {companies.map(c => (
                    <option key={c.id || c._id} value={c.companyName}>{c.companyName}</option>
                  ))}
                </select>
              </div>
          ...
          <div className="flex items-center gap-6 mb-8 px-1">
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-black text-text-secondary uppercase tracking-widest">Showing</span>
              <span className="text-xs font-black text-text-primary">{feedCounts.total} items</span>
            </div>
            <div className="h-4 w-px bg-gray-100 dark:bg-slate-800" />
            <div className="flex items-center gap-2">
              <div className="w-1.5 h-1.5 rounded-full bg-orange-500" />
              <span className="text-[10px] font-black text-text-secondary uppercase tracking-widest">In Progress</span>
              <span className="text-xs font-black text-orange-500">{feedCounts.inProgress}</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-1.5 h-1.5 rounded-full bg-red-500" />
              <span className="text-[10px] font-black text-text-secondary uppercase tracking-widest">Overdue</span>
              <span className="text-xs font-black text-red-500">{feedCounts.overdue}</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
              <span className="text-[10px] font-black text-text-secondary uppercase tracking-widest">Completed</span>
              <span className="text-xs font-black text-emerald-500">{feedCounts.paid}</span>
            </div>
          </div>
...
const StatusBadge = ({ status }: { status: string }) => {
  const styles: any = {
    Paid: 'bg-green-50 dark:bg-emerald-900/20 text-green-600 dark:text-emerald-400 border border-green-100 dark:border-emerald-900/30',
    Pending: 'bg-yellow-50 dark:bg-yellow-900/10 text-yellow-600 dark:text-yellow-400 border border-yellow-100 dark:border-yellow-900/20',
    Verified: 'bg-blue-50 dark:bg-blue-900/10 text-blue-600 dark:text-blue-400 border border-blue-100 dark:border-blue-900/20',
    Approved: 'bg-indigo-50 dark:bg-indigo-900/10 text-indigo-600 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-900/20',
    'Payment Initiated': 'bg-orange-50 dark:bg-orange-900/10 text-orange-600 dark:text-orange-400 border border-orange-100 dark:border-orange-900/20',
    'Payment Confirmed': 'bg-purple-50 dark:bg-purple-900/10 text-purple-600 dark:text-purple-400 border border-purple-100 dark:border-purple-900/20',
    'Tally Entry': 'bg-cyan-50 dark:bg-cyan-900/10 text-cyan-600 dark:text-cyan-400 border border-cyan-100 dark:border-cyan-900/20',
    Overdue: 'bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 border border-red-100 dark:border-red-900/30',
