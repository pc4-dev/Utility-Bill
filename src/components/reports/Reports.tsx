...
import { Bill, BillType, Company } from '../../types';
...
export const Reports: React.FC = () => {
  ...
  useEffect(() => {
    fetchReports();
  }, [filters.year, filters.month, filters.category, filters.subcategory, filters.documentType, filters.status, filters.property, filters.utilityType, filters.company]);
    ...
    const safeAutoTable = (options: any) => {
      try {
        if (typeof autoTable === 'function') {
          ...
          console.warn('jsPDF-AutoTable plugin not found. Falling back to simple text.');
          // Simple fallback could be added here if needed
        }
      } catch (err) {
    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm transition-colors">
        <div className="flex items-center justify-between mb-6">
          ...
          <button 
            onClick={() => setFilters({ startDate: '', endDate: '', category: '', subcategory: '', documentType: '', status: '', month: '', year: '2026', property: '', utilityType: '', company: '' })}
          ...
          <div className="space-y-1.5">
            ...
            >
              ...
              {companies.map(company => (
                <option key={company._id || company.id} value={company.companyName} className="bg-white dark:bg-slate-900">
                  {company.companyName}
                </option>
              ))}
            </select>
