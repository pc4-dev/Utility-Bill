...
export const UserForm: React.FC<UserFormProps> = ({ isOpen, onClose, onSubmit, initialData }) => {
  ...
  useEffect(() => {
    if (initialData) {
      const isPredefined = ['ADMIN', 'MANAGER', 'EA', 'DATA_ENTRY', 'VERIFIER', 'APPROVER', 'GOV_TAX_ENTRY', 'INSURANCE_ENTRY', 'ACCOUNT_MANAGEMENT', 'ACCOUNT_MANAGER', 'ACCOUNT_EXECUTIVE_2'].includes(initialData.role);
      setFormData({
                ...
                <div className="space-y-4">
                  ...
                  {isCustomRole ? (
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      {(['ADMIN', 'MANAGER', 'EA', 'DATA_ENTRY', 'VERIFIER', 'APPROVER', 'GOV_TAX_ENTRY', 'INSURANCE_ENTRY', 'ACCOUNT_MANAGEMENT', 'ACCOUNT_MANAGER', 'ACCOUNT_EXECUTIVE_2'] as Role[]).map((role) => (
                        <button
                          key={role}
                          type="button"
                        ...
                        >
                          ...
                          <span className="text-[9px] font-black uppercase tracking-wider text-center leading-tight">
                            {role === 'GOV_TAX_ENTRY' ? 'Gov Tax' : 
                             ...
                             role === 'ACCOUNT_EXECUTIVE_2' ? 'Acc Exec 2' :
                             role.replace('_', ' ')}
                          </span>
                        </button>
                      ))}
