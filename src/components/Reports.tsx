...
export const Reports: React.FC<ReportsProps> = ({ bills }) => {
  ...
  useEffect(() => {
    const fetchCompanies = async () => {
      try {
        ...
        console.error("Error fetching companies for reports:", err);
      }
    };
    fetchCompanies();
  }, []);
  ...
  const filteredData = useMemo(() => {
    return bills.filter(bill => {
      const date = new Date(bill.due_date || bill.dueDate || '');
      ...
      const matchesType = appliedFilters.billType ? (bill.bill_type === appliedFilters.billType || bill.utilityType === appliedFilters.billType) : true;
      const matchesCompany = appliedFilters.company ? (bill.company_name === appliedFilters.company || bill.companyName === appliedFilters.company) : true;

      return matchesStartDate && matchesEndDate && matchesType && matchesCompany;
    });
    ...
    >
      ...
      <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-orange-50 dark:border-slate-800 transition-colors">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
          ...
          <div>
            <label className="text-xs font-bold text-text-secondary mb-2 block uppercase tracking-wider transition-colors">Company</label>
            <select 
              className="w-full p-3 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl text-sm text-text-primary focus:ring-2 focus:ring-orange-500 outline-none transition-colors"
            ...
            >
              ...
              {companies.map(company => (
                <option key={company} value={company} className="bg-white dark:bg-slate-900">{company}</option>
              ))}
            </select>
          </div>
          <button 
            ...
            {breakdownByType.length > 0 ? breakdownByType.map(([type, data]) => (
              <div key={type} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-slate-800/50 rounded-2xl transition-colors">
                ...
                <p className={cn(
                  ...
                  data.total < 0 ? "text-red-600 dark:text-red-400" : "text-text-primary"
                )}>{formatCurrency(data.total)}</p>
              </div>
            )) : (
              ...
              return (
                <div key={monthKey} className="bg-gray-50 dark:bg-slate-800/50 p-4 rounded-2xl text-center border border-transparent hover:border-orange-200 dark:hover:border-orange-500/30 transition-all transition-colors">
                  ...
                  <p className={cn(
                    ...
                    total < 0 ? "text-red-600" : "text-orange-600 dark:text-orange-400"
            ...
            <tbody>
              {sortedData.map((bill, idx) => (
                <tr key={bill.id} className={cn("group transition-all", idx % 2 === 0 ? "bg-white dark:bg-slate-900" : "bg-gray-50/50 dark:bg-slate-800/30")}>
                  ...
                  <td className={cn(
                    "px-4 py-4 text-sm font-black text-right border-y border-r border-transparent group-hover:border-orange-100 dark:group-hover:border-orange-500/30 rounded-r-xl transition-colors",
                    bill.total_amount < 0 ? "text-red-600 dark:text-red-400" : "text-text-primary"
                  )}>
    ...
    <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl shadow-sm border border-orange-50 dark:border-slate-800 flex items-center gap-4 transition-colors">
      ...
      <div>
        ...
        <p className={cn(
          ...
          value.startsWith('-') || value.includes('₹-') ? "text-red-600 dark:text-red-400" : "text-text-primary"
        )}>{value}</p>
      </div>
    </div>
  );
