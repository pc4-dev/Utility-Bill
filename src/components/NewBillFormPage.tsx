...
import { Bill, Project, BillStatus, BillType, Company } from '../types';
...
export const NewBillFormPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        const [loadedProjects, loadedBillTypes, loadedBills, loadedCompanies] = await Promise.all([
          api.getProjects(),
          api.getBillTypes(),
          api.getBills(),
          api.getCompanies()
        ...
        if (loadedCompanies.length > 0 && !formData.companyName) {
          setFormData(prev => ({ ...prev, companyName: loadedCompanies[0].companyName }));
        }
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.saveBill(newBillData, user ? { name: user.name, role: user.role } : undefined);
