...
export const BillForm: React.FC<BillFormProps> = ({ onClose, onSubmit }) => {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      setCompanies(loadedCompanies.filter(c => c.status === 'Active'));
  ...
  useEffect(() => {
    ...
    const performCheck = async () => {
      if (formData.bill_number && formData.service_provider) {
        try {
          const res = await api.checkDuplicate({
            billNumber: formData.bill_number,
            serviceProvider: formData.service_provider,
            utilityType: formData.bill_type === 'Others' ? formData.customUtilityType : formData.bill_type,
            month: formData.bill_month,
            year: formData.bill_year
          });
  ...
  }, [formData.bill_number, formData.service_provider, formData.bill_type, formData.customUtilityType, formData.bill_month, formData.bill_year]);
                ...
                <div>
                  ...
                  >
                    ...
                    {companies.map(company => (
                      <option key={company.id || company._id} value={company.companyName} className="dark:bg-slate-900">
                        {company.companyName}
                      </option>
                    ))}
