...
import { Bill, UtilityType, Project, BillType, Company } from '../types';
import { 
  X, 
  Upload, 
  IndianRupee, 
...
export const PublicBillForm: React.FC = () => {
  ...
  // Real-time duplicate check for Public Form
  useEffect(() => {
    let timeoutId: NodeJS.Timeout;

    const performCheck = async () => {
      const telecomTypes = ['Telecom', 'Data (Internet)', 'Landline', 'Mobile Recharge'];
      const isTelecom = telecomTypes.includes(formData.utilityType || '');
      const isSolar = formData.utilityType === 'Solar Bill';
      
      const hasTelecomInfo = isTelecom && (formData.accountNumber || formData.mobileNumber);
      const hasSolarInfo = isSolar && formData.consumerNumber;
      const hasGeneralInfo = formData.billNumber && formData.serviceProvider;

      if (hasTelecomInfo || hasSolarInfo || hasGeneralInfo) {
        try {
          ...
          if (isTelecom) {
            ...
            checkParams.billingPeriod = `${formData.month}-${formData.year}`;
          } else if (isSolar) {
          ...
          if (dupRes.ok) {
            ...
            setDuplicateFound(dupData.duplicate);
            setDuplicateMessage(dupData.message || '');
          }
        } catch (err) {
  ...
  }, [formData.utilityType, formData.billNumber, formData.serviceProvider, formData.accountNumber, formData.mobileNumber, formData.consumerNumber, formData.month, formData.year]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const [projectsRes, billTypesRes, companiesRes] = await Promise.all([
          fetch('/api/public/projects', { credentials: 'include' }),
          fetch('/api/public/bill-types', { credentials: 'include' }),
          fetch('/api/public/companies', { credentials: 'include' })
        ]);
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
        ...
        >
          ...
          <h2 className="text-3xl font-black text-gray-900 mb-4 tracking-tight">Data submitted successfully.</h2>
          ...
          <div className="space-y-4">
            <button 
              ...
              className="w-full py-4 bg-orange-600 text-white rounded-2xl font-bold text-lg hover:bg-orange-700 transition-all shadow-xl shadow-orange-100 flex items-center justify-center gap-3"
            >
      ...
      <div className="max-w-[840px] mx-auto">
        ...
        >
          <form onSubmit={handleSubmit} className="p-8 md:p-12 space-y-16">
                ...
                <div className="space-y-2.5">
                  <label className="text-[13px] font-bold text-gray-700 ml-1">Company Name</label>
                    ...
                    >
                      ...
                      {companies.map(c => (
                        <option key={c.id || (c as any)._id} value={c.companyName}>
                          {c.companyName}
                        </option>
                      ))}
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-5">
                  ...
                  >
                    ...
                    <p className="text-[15px] font-bold text-gray-900">Drop files or Paste (Ctrl+V)</p>
            ...
            <AnimatePresence>
              {duplicateFound && (
                <motion.div 
                  ...
                  className="p-5 bg-red-50 border border-red-100 rounded-2xl flex items-center gap-4 text-red-600 mb-6"
                >
                  <AlertCircle className="w-6 h-6" />
                  <p className="text-[14px] font-bold">{duplicateMessage || "This bill appears to be a duplicate."}</p>
                </motion.div>
              )}
            ...
            <button 
              type="submit"
              disabled={isUploading || duplicateFound}
