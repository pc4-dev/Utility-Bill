...
export const AnalyticsDashboard: React.FC<AnalyticsDashboardProps> = ({ bills, chartsData, projects = [] }) => {
  ...
  const companyNames = useMemo(() => Array.from(new Set(bills.map(b => b.company_name))), [bills]);
  const statuses = ['Pending', 'Overdue', 'Paid', 'Payment Confirmed', 'Tally Entry'];
  ...
  const filteredBills = useMemo(() => {
    return bills.filter(bill => {
      ...
      const matchesCompany = appliedFilters.company ? bill.company_name === appliedFilters.company : true;
      
      let matchesStatus = true;
      if (appliedFilters.status === 'PAID' || appliedFilters.status === 'Paid') {
        matchesStatus = ['Paid', 'PAID', 'Payment Confirmed', 'Tally Entry'].includes(bill.status || '');
      } else if (appliedFilters.status === 'PENDING' || appliedFilters.status === 'Pending') {
        matchesStatus = !['Paid', 'PAID', 'Payment Confirmed', 'Tally Entry'].includes(bill.status || '');
      } else if (appliedFilters.status) {
        matchesStatus = bill.status === appliedFilters.status;
      }

      const matchesMonth = appliedFilters.month ? new Date(bill.due_date).getMonth() + 1 === parseInt(appliedFilters.month) : true;
      const matchesYear = appliedFilters.year ? new Date(bill.due_date).getFullYear() === parseInt(appliedFilters.year) : true;

      return matchesStart && matchesEnd && matchesType && matchesProject && matchesStatus && matchesMonth && matchesYear;
    });
  ...
  const kpis = useMemo(() => {
    ...
    const paidBillsCount = filteredBills.filter(b => ['Paid', 'PAID', 'Payment Confirmed', 'Tally Entry'].includes(b.status || '')).length;
    ...
    <div className="space-y-8 mt-8">
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-orange-50">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4 items-end">
          ...
          <div>
            <label className="text-xs font-bold text-gray-500 mb-2 block uppercase tracking-wider">Year</label>
            <select 
              className="w-full p-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-orange-500"
              value={filters.year}
          ...
          <div>
            <label className="text-xs font-bold text-gray-500 mb-2 block uppercase tracking-wider">Company</label>
            <select 
              className="w-full p-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-orange-500"
