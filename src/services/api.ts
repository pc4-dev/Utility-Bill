import { Bill, User, Project, AppNotification, BillType, Company } from '../types';
...
export const api = {
  ...
  getDashboardSummary: async (categories?: string[], company?: string): Promise<any> => {
    try {
      const queryParams = new URLSearchParams();
      if (categories) queryParams.append('categories', categories.join(','));
      if (company) queryParams.append('company', company);
      
      const url = `${API_BASE}/dashboard/summary?${queryParams.toString()}`;
  ...
  saveBill: async (bill: Partial<Bill>, triggeredBy?: { name?: string, role?: string }): Promise<Bill> => {
    ...
    try {
      const res = await fetchWithRetry(url, {
        ...
        body: JSON.stringify({ ...bill, triggeredBy }),
        credentials: 'include'
      });
  ...
  deleteBill: async (id: string | number, triggeredBy?: { name?: string, role?: string }): Promise<void> => {
    try {
  ...
  tallyEntry: async (id: string | number, remarks: string, userName?: string, userRole?: string, tallyData?: any): Promise<Bill> => {
    try {
      const res = await fetchWithRetry(`${API_BASE}/bills/${id}/tally-entry`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ remarks, userName, userRole, tallyData }),
        credentials: 'include'
      });
      return handleResponse(res, 'Failed to complete tally entry');
    } catch (err) {
      console.error('Tally entry network error:', err);
      throw err;
    }
  },
  ...
  getCompanies: async (): Promise<Company[]> => {
    const res = await fetchWithRetry(`${API_BASE}/companies`, { credentials: 'include' });
    return handleResponse(res, 'Failed to fetch companies');
  },

  saveCompany: async (company: Partial<Company>): Promise<Company> => {
    const method = company.id || company._id ? 'PUT' : 'POST';
    const url = company.id || company._id ? `${API_BASE}/companies/${company.id || company._id}` : `${API_BASE}/companies`;
    const res = await fetchWithRetry(url, {
      method,
      headers: { 'Content-Type': 'application/json' },
    ...
    return handleResponse(res, 'Failed to save company');
  },

  deleteCompany: async (id: string): Promise<void> => {
    const res = await fetchWithRetry(`${API_BASE}/companies/${id}`, { method: 'DELETE', credentials: 'include' });
    if (!res.ok) {
      await handleResponse(res, 'Failed to delete company');
    }
  },
  ...
  // File Upload
  uploadFiles: async (formData: FormData, signal?: AbortSignal): Promise<{ files: { url: string, name: string, type: string }[] }> => {
  ...
  uploadTelecom: async (formData: FormData, signal?: AbortSignal): Promise<{ 
    fileUrl: string, 
    fileName: string, 
    mimeType: string, 
    extractedData: any,
  ...
  uploadGovernmentBill: async (formData: FormData, signal?: AbortSignal): Promise<{ 
    fileUrl: string, 
    fileName: string, 
    mimeType: string, 
    extractedData: any,
  ...
  uploadElectricity: async (formData: FormData, signal?: AbortSignal): Promise<{ 
    fileUrl: string, 
    fileName: string, 
    mimeType: string, 
    extractedData: any,
  ...
  uploadSolar: async (formData: FormData, signal?: AbortSignal): Promise<{ 
    fileUrl: string, 
    fileName: string, 
    mimeType: string, 
    extractedData: any,
  ...
  uploadInsurance: async (formData: FormData, signal?: AbortSignal): Promise<{ 
    fileUrl: string, 
    fileName: string, 
    mimeType: string, 
    extractedData: any,
  ...
  uploadPollution: async (formData: FormData, signal?: AbortSignal): Promise<{ 
    fileUrl: string, 
    fileName: string, 
    mimeType: string, 
    extractedData: any,
  ...
  // Expenses
  getExpenses: async (): Promise<Bill[]> => {
    const res = await fetchWithRetry(`${API_BASE}/expenses`, { credentials: 'include' });
    return handleResponse(res, 'Failed to fetch expenses');
  },

  uploadExpense: async (formData: FormData, signal?: AbortSignal): Promise<{ 
    fileUrl: string, 
    fileName: string, 
    mimeType: string 
  }> => {
    await api.ensureSession();
    const res = await fetchWithRetry(`${API_BASE}/expenses/upload`, {
      method: 'POST',
      body: formData,
      credentials: 'include',
      signal
    });
    return handleResponse(res, 'Expense bill upload failed');
  },

  saveExpense: async (expense: Partial<Bill>): Promise<Bill> => {
    const res = await fetchWithRetry(`${API_BASE}/expenses/save`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(expense),
      credentials: 'include'
    });
    return handleResponse(res, 'Failed to save expense');
  },

  submitPublicExpense: async (expense: Partial<Bill>): Promise<Bill> => {
    const res = await fetchWithRetry(`${API_BASE}/expenses/public`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(expense)
    });
    return handleResponse(res, 'Public expense submission failed');
  },

  getPublicProjects: async (): Promise<Project[]> => {
    const res = await fetchWithRetry(`${API_BASE}/public/projects`, { credentials: 'include' });
    return handleResponse(res, 'Failed to fetch public projects');
  },

  getPublicBillTypes: async (): Promise<BillType[]> => {
    const res = await fetchWithRetry(`${API_BASE}/public/bill-types`, { credentials: 'include' });
    return handleResponse(res, 'Failed to fetch public bill types');
  },
  
  getPublicCompanies: async (): Promise<Company[]> => {
    const res = await fetchWithRetry(`${API_BASE}/public/companies`, { credentials: 'include' });
    return handleResponse(res, 'Failed to fetch public companies');
  }
};
