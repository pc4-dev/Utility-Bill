...
export async function generateContentWithRetry(params: GenerateContentParameters, signal?: AbortSignal) {
...
export async function extractExpenseData(fileData: string, mimeType: string, signal?: AbortSignal) {
  try {
    const prompt = `
      Analyze this image or PDF document which is an expense receipt/invoice.
      Extract the following information and return it ONLY as a JSON object:
      {
        "expenseTitle": "Brief descriptive title",
        "amount": 123.45,
        "billDate": "YYYY-MM-DD",
        "vendorName": "Name of the shop or company",
        "location": "City or area if mentioned",
        "description": "Short summary of items bought or services rendered"
      }
      If a field cannot be found, use an empty string or null for numbers.
